...
export default function Home() {
  ...
  const [currentLocation,setCurrentLocation] = useState("January")

  const currentPoints = useMemo(() => points.filter(x => currentLocation === "Both" || x.location === currentLocation),[points,currentLocation]);
  ...
  const onClickSvg = evt => {
    try {
      ...
      if (!points.some(item => item.x === x && item.y === y)) {
        setPoints([...points,{ location: currentLocation,x,y }])
      }
    } catch (error) {
      console.log(error)
    }
    ...
    <div className="container">
      <div className="row pt-2">
        <div className="col-12 d-flex align-items-center justify-content-center gap-1">
          <button className='btn btn-primary' onClick={() => locationClick("January")}>January</button>
          <button className='btn btn-primary' onClick={() => locationClick("February")}>February</button>
          <button className='btn btn-primary' onClick={() => locationClick("Both")}>Both</button>
        </div>
      ...
      <div className="row">
        <div className="col-6">
          <div style={{ position: "relative" }}>
            <svg xmlns="http://www.w3.org/2000/svg" version="1.0" preserveAspectRatio="xMidYMid meet" viewBox="361.14 43.29 599.57 614.15" onClick={onClickSvg}>
            ...
            {currentPoints.map(item => <span
              ...
              style={{
                ...
                left: item.x,
                top: item.y,
                background: item.location === "January" ? "red" : "green",
                height: "5px",
                width: "5px",
                borderRadius: "5px"
              }}
